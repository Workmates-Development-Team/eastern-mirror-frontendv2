...
function TiptapImage(props: NodeViewProps) {
  ...
  const [imageLink, setImageLink] = useState(node.attrs.link || "")
  ...
  function handleLinkBlur() {
    const url = getUrlFromString(imageLink)
    if (url) {
      ...
      <div className={cn("group relative flex flex-col rounded-md", resizing && "")}>
        ...
        {editingLink && (
          <Input
            ...
            className="mt-2 text-center text-sm focus:ring-0"
            placeholder="https://example.com"
            autoFocus
          />
        )}
        {!editingLink && node.attrs.link && (
          <div className="mt-2 text-center text-sm text-blue-500 underline">{node.attrs.link}</div>
        )}
          ...
          >
            ...
            <DropdownMenu open={openedMore} onOpenChange={setOpenedMore}>
              ...
              <DropdownMenuContent align="start" alignOffset={-90} className="mt-1 text-sm">
                ...
                <DropdownMenuItem onClick={() => setEditingLink(true)}>
                  <LinkIcon className="mr-2 size-4" /> {node.attrs.link ? "Edit Link" : "Add Link"}
                </DropdownMenuItem>
                <DropdownMenuSub>
                  <DropdownMenuSubTrigger>
